...
export async function requireTina(request:Request){const env=workerEnv();const token=request.headers.get('authorization');if(!token||!env.PUBLIC_TINA_CLIENT_ID)return false;const authorized=await isUserAuthorized({clientID:env.PUBLIC_TINA_CLIENT_ID,token});return Boolean(authorized)}
